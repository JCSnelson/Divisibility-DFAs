...
\begin{defn} 
  ...
  \[val(c_1\circ c_2\circ\dots\circ c_n)=2^0\times c_1+2^1\times c_2+\dots+2^{n-1}\times c_n\]
\end{defn}
...
\end{defn}
And so with respect to our problem we have that two binary strings $s_x,s_y$ are non distinguishable if and only if  $\forall s_w \in \{0,1\}^*$ $val(s_x\circ s_w)\equiv 0 \iff val(s_x\circ s_w)\equiv 0$
\subsection{Initial ND Criteria}
\begin{defn}
  We define the residue of a string with respect to a certain base to be 
\end{defn}
\begin{lemma}
